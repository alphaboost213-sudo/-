...
def build_opener_for_url(url: str) -> urllib.request.OpenerDirector:
    """
    Возвращает urllib opener с прокси если URL относится к kapps.pro или
    если задан PROXY_URL.
    Прокси берётся из переменной PROXY_URL или используется gate.vpn:8080
    с кредами из PROXY_USER / PROXY_PASS.
    """
    proxy_url = os.environ.get('PROXY_URL', '').strip()
    proxy_user = os.environ.get('PROXY_USER', 'browser').strip()
    proxy_pass = os.environ.get('PROXY_PASS', 'vorobeych1k1102').strip()
    proxy_host = os.environ.get('PROXY_HOST', 'gate.vpn').strip()
    proxy_port = os.environ.get('PROXY_PORT', '8080').strip()

    use_proxy = False

    # Принудительный прокси для kapps.pro
    from urllib.parse import urlparse as _urlparse
    host = _urlparse(url).hostname or ''
    if host.endswith('kapps.pro'):
        use_proxy = True

    # Или если явно задан PROXY_URL
    if proxy_url:
        use_proxy = True
...
def load_image_from_url(url: str) -> Image.Image:
    """Загружает изображение по URL, используя прокси для kapps.pro."""
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
        'Accept': 'image/webp,image/apng,image/*,*/*;q=0.8',
    }
    opener = build_opener_for_url(url)
...
def download_url_to_temp(url: str, suffix: str = '.bin') -> str:
    ...
    Для kapps.pro использует прокси автоматически."""
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
        'Accept': '*/*',
    }
    ...
    try:
        with opener.open(req, timeout=60) as response, tmp:
            while True:
