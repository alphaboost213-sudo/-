...
# If SOURCE_PROXY_DOMAINS is empty, all source links go through the proxy.
SOURCE_PROXY_URL = os.environ.get('SOURCE_PROXY_URL', '').strip()
SOURCE_PROXY_DOMAINS = tuple(
    domain.strip().lower()
    for domain in os.environ.get('SOURCE_PROXY_DOMAINS', '').split(',')
    if domain.strip()
)
...
def should_use_source_proxy(url: str) -> bool:
    if not SOURCE_PROXY_URL:
        return False
    if not SOURCE_PROXY_DOMAINS:
        return True

    hostname = (urlparse(url).hostname or '').lower()
    return any(
        hostname == domain or hostname.endswith(f'.{domain}')
        for domain in SOURCE_PROXY_DOMAINS
    )


def open_source_url(req: urllib.request.Request, source_url: str, timeout: int):
    if not should_use_source_proxy(source_url):
        return urllib.request.urlopen(req, timeout=timeout)

    opener = urllib.request.build_opener(
        urllib.request.ProxyHandler({
            'http': SOURCE_PROXY_URL,
            'https': SOURCE_PROXY_URL,
        })
    )
...
def load_image_from_url(url: str) -> Image.Image:
    ...
    with open_source_url(req, url, timeout=15) as response:
        data = response.read()
...
def download_url_to_temp(url: str, suffix: str = '.bin') -> str:
    ...
    try:
        with open_source_url(req, url, timeout=25) as response, tmp:
            while True:
...
def stats():
    return jsonify({
        ...
        'source_proxy_enabled': bool(SOURCE_PROXY_URL),
        'source_proxy_domains': SOURCE_PROXY_DOMAINS,
